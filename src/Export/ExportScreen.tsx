...
export default class ExportScreen extends Component<Props, State> {
        ...
        <ScrollView>
          <View>
            ...
            <Button
              ...
              title={"Import Data"}
            />
            <Text style={styles.H3}>{this.props.t("export")}</Text>
            <Text style={styles.text}>{this.props.t("export-blurb")}</Text>
            <Button
              buttonStyle={styles.roundButton}
              titleStyle={styles.buttonText}
  ...
  public clearFolder() {
    const path = RNFS.DocumentDirectoryPath + "/Inbox";
  ...
  private async exportField(farmKey: string, password: string) {
    ...
    database.exportFarm(farmKey).then(async farm => {
      ...
      const salt64 = this.Base64.encode(
        String.fromCharCode.apply(null, saltbytes)
      );

      const ivbytes = await generateSecureRandom(16);
      const iv64 = this.Base64.encode(String.fromCharCode.apply(null, ivbytes));
      ...
      const outputString = `${salt64}:${iv64}:${mac64}:${encrypted64}`;
  ...
  private async importJson() {
    ...
    Alert.alert("Import this farm?", message, [
      ...
      },
      {
        text: "OK",
        onPress: async () => {
          const results = await database.importFarm(data);
